...
public class SaleOrderServiceImpl implements SaleOrderService {
  ...
  @Override
  public List<SaleOrderLine> manageComplementaryProductSOLines(SaleOrder saleOrder)
      throws AxelorException {
    ...
    List<SaleOrderLine> removableComplementarySOLines = new ArrayList<>();
    if (CollectionUtils.isEmpty(saleOrder.getClientPartner().getComplementaryProductList())) {
      removableComplementarySOLines =
          saleOrder.getSaleOrderLineList().stream()
              .filter(line -> line.getMainSaleOrderLine() != null)
              .collect(Collectors.toList());
      removableComplementarySOLines.stream()
          .forEach(line -> saleOrder.removeSaleOrderLineListItem(line));
      return null;
    }
    ...
    for (SaleOrderLine saleOrderLine : saleOrder.getSaleOrderLineList()) {
      removableComplementarySOLines.addAll(
          saleOrderLineService.manageComplementaryProductSaleOrderLine(
              saleOrderLine, saleOrder, complementaryProducts, newComplementarySOLines));
    }
    removableComplementarySOLines.stream()
        .forEach(line -> saleOrder.removeSaleOrderLineListItem(line));
